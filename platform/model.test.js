...
test('Default port rows carry a sourced country, terminal, notes and numeric limits where published',()=>{const s=M.initial();assert.equal(s.portRecords.length,29,'12 single-row call ports, five Murmansk berths and twelve delivery positions');assert.equal(new Set(s.portRecords.map(p=>p.name)).size,25);
// A delivery position is not a call port: it carries a place and a country, and nothing about it is assumed.
const delivery=p=>M.portProfileOf(p.name)?.delivery===true;
assert.equal(s.portRecords.filter(delivery).length,12);
for(const p of s.portRecords.filter(delivery)){assert.ok(Number.isFinite(p.lat)&&Number.isFinite(p.lon),'no position for '+p.name);assert.ok(p.country.trim(),'country missing for '+p.name);assert.equal(p.terminal,'','no terminal is claimed for '+p.name);assert.equal(p.waterDensity,null,'no density is assumed for '+p.name);assert.match(p.notes,/^Position for routing the ballast approach\./,'the row says what it is: '+p.name);for(const k of M.PORT_LIMIT_FIELDS)assert.equal(p[k],null,k+' must stay empty for the unvetted position '+p.name);}
for(const p of s.portRecords.filter(p=>!delivery(p))){assert.ok(p.terminal.trim(),'terminal missing for '+p.name);assert.ok(p.notes.trim(),'notes missing for '+p.name);assert.ok(p.country.trim(),'country missing for '+p.name);for(const k of M.PORT_LIMIT_FIELDS)assert.ok(p[k]===null||M.ok(p[k],true),k+' must be a positive number or null for '+p.name);}const murmansk=s.portRecords.filter(p=>p.name==='Murmansk');assert.deepEqual(murmansk.map(p=>[p.berth,p.maxDraft,p.maxLoa,p.maxBeam,p.maxAirDraft]),[['Berth 4',11,230,32.2,14.5],['Berth 6',7,120,16,14.5],['Berth 7',10,225,32,14.5],['Berth 9/10',10.5,240,36,14.5],['Berth 13',12.5,240,36,14.5]]);assert.ok(murmansk.every(p=>p.terminal==='Murmansk Sea Commercial Port'),'every berth row keeps the terminal');const spb=s.portRecords.find(p=>p.name==='St. Petersburg');assert.equal(spb.maxDraft,11,'user-supplied fresh water draft');assert.equal(spb.maxLoa,null,'an unpublished limit stays empty rather than zero');assert.equal(s.portRecords.find(p=>p.name==='Santarem').maxDwt,60000);assert.ok(murmansk.every(p=>p.country==='Russia'));assert.equal(s.portRecords.find(p=>p.name==='Suape').country,'Brazil');});
...
test('Every default berth carries one assumed water density inside the selectable range',()=>{
 ...
 // Delivery positions are excluded: no cargo is worked there, so no density is assumed for them.
 for(const p of s.portRecords.filter(p=>!M.portProfileOf(p.name)?.delivery)){
