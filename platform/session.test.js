...
function asyncStorage(seed={}){
 ...
 for(const name of ['ready','user','loadCatalogs','saveCatalogs','listSales','saveSale','deleteSale',
  ...
  wrapped[name]=async(...args)=>{calls.push(name);await Promise.resolve();return inner[name](...args);};
 }
...
test('A new workspace stores its seeded registers when it opens',async()=>{
 ...
 const stored=await storage.loadCatalogs();
 assert.deepEqual(stored.document.cargoTypes,state.cargoTypes,'the reference cargo register is on disk, not only on screen');
 assert.equal((await storage.listSales()).length,state.sales.length);
});

test('A keystroke in the voyage does not rewrite the shared catalogs',async()=>{
 const storage=asyncStorage();
 const session=Session.create(storage,{fresh:()=>M.initial()});
 const {state}=await session.open();
 ...
 assert.equal(storage.calls.filter(c=>c==='saveCatalogs').length,0,'the organisation catalogs were not touched');
 assert.equal(storage.calls.filter(c=>c==='saveVoyage').length,1);

 storage.calls.length=0;
 state.cargoTypes[0].sf=1.11;
 assert.equal((await session.save(state)).ok,true);
 assert.equal(storage.calls.filter(c=>c==='saveCatalogs').length,1,'a catalog edit is written once');
});
...
test('Only the sale that changed is written, and a removed sale is deleted',async()=>{
 ...
 assert.equal((await session.save(state)).ok,true);
 assert.equal(storage.calls.filter(c=>c==='saveSale').length,1,'the untouched deal is not rewritten');

 const removed=state.sales.pop();
 storage.calls.length=0;
 assert.equal((await session.save(state)).ok,true);
 assert.equal(storage.calls.filter(c=>c==='deleteSale').length,1);
 const left=await storage.listSales();
