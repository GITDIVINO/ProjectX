...
// The water density a user picks in PORT is validated and clearable on its own.
test('Berth water density is validated, stored and clearable',()=>{
 const s=base();P.ensure(s);
 const berth=s.portRecords.find(p=>p.name===s.ports[0].name);s.ports[0].planning.berthId=berth.id;
 const idx=s.portRecords.findIndex(p=>p.id===berth.id);
 M.updatePortRecord(s,idx,'waterDensity','1.02');assert.equal(s.portRecords[idx].waterDensity,1.02);
 assert.throws(()=>M.updatePortRecord(s,idx,'waterDensity','1020'),'a density outside the physical range is refused');
 M.updatePortRecord(s,idx,'waterDensity','');assert.equal(s.portRecords[idx].waterDensity,null,'clearing leaves unknown, not zero');
});
