...
(async()=>{const browser=await chromium.launch({headless:true});try{
 ...
 await seed(base());await calc();assert.match(await head(),/35,692\.00 t estimated restricted intake/i);
 ...
 await page.reload();assert.equal(await page.locator('#intake-calculator').evaluate(e=>e.open),false);assert.match(await head(),/35,692.00 t/i);
 await open();await edit('deductions.fuel','951');assert.match(await head(),/not calculated/i);await calc();assert.match(await head(),/35,691.00 t/i);
 await edit('holds.0.volume','100');assert.match(await page.locator('.cubic-intake').innerText(),/43,202.22 t/);
 await edit('holds.0.volume','');assert.match(await head(),/35,691.00 t DWT only · cubics not checked/i);assert.doesNotMatch(await head(),/restricted intake/);
 ...
 await seed(s);await calc();assert.match(await head(),/43,538.48 t/i);assert.match(await page.locator('.draft-formula').innerText(),/draft 12.8 × 1.025 ÷ ρ 1 = 13.12 m/);
 ...
 await page.locator('#tab-ports').click();await page.locator(`[data-path="portRecords.${idx}.waterDensity"]`).selectOption('1.025');await page.locator('#tab-planner').click();await calc();assert.match(await head(),/45,119.80 t/i);
