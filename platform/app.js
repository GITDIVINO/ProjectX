...
// One quiet line where the result would be: the fields still missing, named as the form names them.
// The model stops at the first block it cannot pass, so the line states what is known now, not a full audit.
const MISSING_LABEL={distance:'total distance',eca:'ECA distance',speed:'speed',margin:'weather',burn:'consumption outside ECA','consumption ECA':'consumption in ECA',aux:'Aux consumption',turn:'turn time',extra:'waiting',da:'DA',working:'working consumption',idle:'idle consumption'};
function missingLine(errors){
 if(!chosen().length||!errors?.length)return '';
 const named=[...new Set(errors.map(e=>{const at=e.lastIndexOf(': ');const key=at<0?e:e.slice(at+2);const label=MISSING_LABEL[key];return label?e.slice(0,at+2)+label:e;}))];
 const shown=named.slice(0,6),rest=named.length-shown.length;
 return `<p class="chain-missing">Voyage not calculated · ${esc(shown.join(', '))}${rest?` and ${rest} more`:''}</p>`;
}
...
html+=`<section><div class="heading"><h2>4. Voyage calculation</h2></div>${b?voyageChain(b,ship):missingLine(r.errors)}`;
