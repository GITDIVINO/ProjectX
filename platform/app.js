...
html+=`<section><div class="heading"><h2>2. Vessel and rotation</h2></div><div class="vessel-choice"><label class="field">Vessel${select('vesselId','Vessel',(state.vesselProfiles||M.VESSELS).map(v=>[v.id,v.name]))}</label><span class="muted vessel-summary">${fmt(M.vesselOf(state)?.dwt,0)} DWT · ${M.vesselOf(state)?.holds} holds · ${esc(M.vesselOf(state)?.model)} · LOA ${fmt(M.vesselOf(state)?.loa)} m · Beam ${fmt(M.vesselOf(state)?.beam)} m · Draft ${fmt(M.vesselOf(state)?.draft,2)} m · TPC ${fmt(M.vesselOf(state)?.tpc)} · Grain ${fmt(M.vesselOf(state)?.grain,0)} m³</span></div>${intakeCalculator(ship)}<div class="rotation-grid">${rotationList('Loading',true)}${rotationList('Discharge',false)}</div></section>`;
