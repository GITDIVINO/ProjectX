...
html+=table(['Leg','Total, NM','Of which ECA, NM','Speed, kn','Weather, % time','Outside ECA, t/day','In ECA, t/day','Aux, t/day','Days'],displayedLegs.map(({l,path})=>`<tr><td class="name">${esc(l.from)} → ${esc(l.to)}</td>${['distance','eca','speed','margin','burn','ecaBurn','aux'].map(k=>`<td>${input(path+'.'+k,l.from+' '+k)}</td>`).join('')}<td>${fmt(b?.legs.find(x=>x.from===l.from&&x.to===l.to)?.days??M.legDays(l),3)}</td></tr>`));// The calendar period belongs to manual handling terms only: the column appears with the call that needs it.
const manualTerms=activePorts.some(p=>p.terms==='manual');
html+=`<h3>Ports</h3>`+table(['Port','Cargo, MT','Handling rate, t/day','Handling terms',...(manualTerms?['Calendar, days']:[]),'Turn time, h','Waiting, h','DA, USD','Days'],activePorts.map(p=>{const i=state.ports.indexOf(p),path='ports.'+i;return `<tr><td>${esc(p.name)}</td><td>${fmt(lots.filter(l=>l.loadPort===p.name||l.port===p.name).reduce((n,l)=>n+(l.quantity||0),0),0)}</td><td>${input(path+'.rate',p.name+' handling rate')}</td><td>${select(path+'.terms',p.name+' terms',[['SHINC','24/7 · SHINC'],['manual','Manual calendar']])}</td>${manualTerms?`<td>${input(path+'.calendar',p.name+' calendar days',{disabled:p.terms==='SHINC'})}</td>`:''}<td>${input(path+'.turn',p.name+' turn time')}</td><td>${input(path+'.extra',p.name+' waiting')}</td><td>${input(path+'.da',p.name+' DA')}</td><td>${fmt(b?.ports.find(x=>x.name===p.name)?.days??M.portDays(state,p),3)}</td></tr>`;}));
