...
const leg=(from,to)=>({from,to,distance:null,distanceSource:null,eca:null,speed:12.5,burn:16,ecaBurn:null,aux:null,margin:0});
...
const PORT_PROFILES={
 ...
 'Murmansk':{lat:68.97,lon:33.05,waterDensity:1.025,country:'Russia',terminal:'Murmansk Sea Commercial Port',maxDraft:12.5,maxLoa:240,maxBeam:36,maxAirDraft:14.5,maxDwt:null,notes:'Air draft 14.5 m at all berths.',berths:[
 ...
 'St. Petersburg':{lat:59.9,lon:30.25,waterDensity:1,country:'Russia',terminal:'Sea Port of Saint Petersburg, First and Second Cargo Areas',maxDraft:11,maxLoa:null,maxBeam:null,maxAirDraft:null,maxDwt:null,notes:'Max draft 11 m in fresh water, supplied by the user 8 September 2026. Operator lists 31 dry cargo berths without published per-berth limits; confirm the assigned berth.'},
 'Ust-Luga':{lat:59.67,lon:28.4,waterDensity:1.004,country:'Russia',terminal:'European Sulphur Terminal / EuroChem Ust-Luga Terminal',maxDraft:13.1,maxLoa:334,maxBeam:null,maxAirDraft:null,maxDwt:null,notes:'EuroChem berth 1: LOA 334 m, draft 13.1 m. Berth 2: LOA 295 m, draft 8.5 m. Status of berths 3-4 to be confirmed.'},
 'Santos':{lat:-23.98,lon:-46.3,waterDensity:1.015,country:'Brazil',terminal:'STS20, Outeirinhos, berths 22/23',maxDraft:11.3,maxLoa:null,maxBeam:null,maxAirDraft:null,maxDwt:null,notes:'Depth 11.3 m in the source study, not a permitted draft: check the current operational draft in the port table. The 283 m face is berths 22 and 23 combined, so it is not a single-ship LOA limit.'},
 'Paranaguá':{lat:-25.5,lon:-48.52,waterDensity:1.015,country:'Brazil',terminal:'TEFER, public fertilizer terminal',maxDraft:12.5,maxLoa:null,maxBeam:null,maxAirDraft:null,maxDwt:null,notes:'General published draft 12.5 m. A 2023 announcement gives 12.8 m at fertilizer berths 209/211; the lower figure is kept until confirmed.'},
 'Itaqui':{lat:-2.57,lon:-44.37,waterDensity:1.015,country:'Brazil',terminal:'COPI / public berths 100-103',maxDraft:12,maxLoa:null,maxBeam:null,maxAirDraft:null,maxDwt:null,notes:'Max draft 12 m in brackish water, supplied by the user 8 September 2026. Confirm the exact berth and the current draft, LOA and beam with EMAP/COPI.'},
 'Santarem':{lat:-2.42,lon:-54.72,waterDensity:1,country:'Brazil',terminal:'Solid Bulk Terminal, Pier 400 berth 401',maxDraft:16,maxLoa:250,maxBeam:null,maxAirDraft:null,maxDwt:60000,notes:'Depth 16 m is the quay depth, not a permitted draft, and the river level governs. Berth about 250 m, up to 60,000 DWT. Confirm fertilizer handling.'},
 'Vitoria':{lat:-20.32,lon:-40.33,waterDensity:1.015,country:'Brazil',terminal:'Vports, Vila Velha terminal',maxDraft:12,maxLoa:null,maxBeam:null,maxAirDraft:null,maxDwt:null,notes:'Max draft 12 m in brackish water, supplied by the user 8 September 2026. Berth assignment and other limits are absent from the public profile used; confirm with Vports.'},
 'Rio Grande':{lat:-32.13,lon:-52.1,waterDensity:1.01,country:'Brazil',terminal:'Yara Brasil Fertilizantes, North/South',maxDraft:12.19,maxLoa:null,maxBeam:null,maxAirDraft:null,maxDwt:null,notes:'South draft 12.19 m, North 10.0 m. The terminal limit prevails over the channel.'},
 'San Francisco do Sul':{lat:-26.24,lon:-48.64,waterDensity:1.02,country:'Brazil',terminal:'Public berth 201 / Bulk Terminal',maxDraft:14,maxLoa:250,maxBeam:null,maxAirDraft:null,maxDwt:null,notes:'Draft 14 m, max LOA 250 m. Confirm the current structural and operational limits.'},
 'Suape':{lat:-8.39,lon:-34.95,waterDensity:1.024,country:'Brazil',terminal:'SUA Graneis, quay 5',maxDraft:15,maxLoa:300,maxBeam:null,maxAirDraft:null,maxDwt:null,notes:'Max operational draft 15 m, max LOA 300 m; quay length 343-344 m. Confirm fertilizer handling.'},
 'Aratu':{lat:-12.8,lon:-38.5,waterDensity:1.022,country:'Brazil',terminal:'TGS, Pier 1 North/South',maxDraft:12,maxLoa:250,maxBeam:null,maxAirDraft:null,maxDwt:null,notes:'Depth 12 m. Pier 1 South max LOA 250 m, North 200 m. Another source page reports a lower South draft: confirmation required.'},
...
// The cubic intake keeps the currently selected cargo mix proportional.  It is a
// planning limit, not a substitute for a hold-by-hold stowage plan.
function intakeLimits(s){
 const volume=cargoVolume(s),vessel=vesselOf(s);
 const dwt=DEDUCTIONS.every(k=>ok(s.deductions[k]))&&ok(vessel?.dwt,true)
  ?R(vessel.dwt).sub(rsum(DEDUCTIONS.map(k=>R(s.deductions[k])))).number():null;
 const cubic=volume.holdTotal!==null&&ok(volume.weightedSf,true)
  ?R(volume.holdTotal).div(volume.weightedSf).number():null;
 return {dwt,cubic,restricted:dwt!==null&&cubic!==null?Math.min(dwt,cubic):null,weightedSf:volume.weightedSf,holdTotal:volume.holdTotal};
}
...
if(new Set(s.holds.map(h=>h.id)).size!==s.holds.length)errors.push('Duplicate hold ID');if(intake===null)warnings.push('Restricted intake DWT is unknown: complete DWT and every deduction');
...
function syncSalesToLots(s){ensureBusinessData(s);
 for(const sale of s.sales)for(const [idKey,nameKey] of [['loadPortId','loadPort'],['dischargePortId','dischargePort']]){const rows=s.portRecords.filter(p=>p.name===sale[nameKey]);if(rows.length&&!rows.some(p=>p.id===sale[idKey]))sale[idKey]=rows[0].id;}
 for(const l of s.lots){const sale=s.sales.find(x=>x.id===l.saleId);if(sale){l.quantity=l.plannedQuantity??sale.quantity;l.loadPort=sale.loadPort;l.port=sale.dischargePort;}}syncRoute(s);}
...
const api={DEDUCTIONS,berthsAt,cargoVolume,intakeLimits,validateSale,updateSale,removePortRecord,updatePortRecord,PORT_PROFILES,portProfileOf,PORT_LIMIT_FIELDS,portLimitBreaches,isBulkCargo,anonymizeProfiles,addVesselType,migrateBaltic,ensureCatalogs,ensureBusinessData,syncSalesToLots,addSale,addSaleToPlanner,applyCargo,applyVessel,VESSELS,vesselOf,moveCall,LOAD_PORT,loadOf,callsOf,syncRoute,CARGO_TYPES,changeLoadPort,addLot,initial,demo,allocate,stowage,compute,stageAllocations,splitCents,ok};if(typeof module!=='undefined'&&module.exports)module.exports=api;else root.ProjectXModel=api;
})(globalThis);
