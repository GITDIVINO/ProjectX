...
const NO_SESSION=/session missing|session_not_found|not authenticated|no user found/i;
...
function create(options={}){
 ...
 return {
  ...
  // Supabase reports "nobody is signed in" as an error rather than as an empty user, and
  // its wording — "Auth session missing!" — is not something to show a person who has
  // simply not signed in yet. Having no session is the ordinary state of the sign-in
  // screen, so it is reported as that and not as a failure.
  async ready(){
   const {data,error}=await client.auth.getUser();
   if(error)return NO_SESSION.test(error.message||'')
    ? {ok:false,reason:'signed-out'} : fail(error,'auth');
