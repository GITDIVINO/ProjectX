...
test('Saved two-cargo state receives seeds once without altering original parcels or edits',()=>{
 const s=M.demo();s.cargoTypes=s.cargoTypes.slice(0,2);delete s.importedCargoSeeds;
...
test('Loose density conversion and sulphur scope retain their separate meanings',()=>{
 ...
 const s=M.initial();assert.equal(s.cargoTypes[1].sf,.95);assert.equal(s.cargoTypes[1].onlyHold,4);assert.equal(s.cargoTypes[0].sf,null);assert.equal(s.cargoTypes[0].group,'');assert.equal(s.lots[1].sf,null);
});
