...
for(const name of ['arithmetic.js','cargo-catalog.js','model.js','market.js','guide.js','sea-data.js','sea-table.js','sea-route.js','planning.js','planner-ui.js','app.js']){
 const source=fs.readFileSync(path.join(dir,name),'utf8');
 new vm.Script(source,{filename:name});
 html=html.replace(`<script src="${name}"></script>`,()=>'<script>'+source.replace(/<\/script/gi,'<\\/script')+'</script>');
}
const scripts=[...html.matchAll(/<script>([\s\S]*?)<\/script>/g)];
if(scripts.length!==11||html.includes('<script src='))throw Error('Incomplete offline build');
