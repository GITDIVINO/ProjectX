...
The current release passes 163 automated tests; the financial engine is checked against 240 independent rational-arithmetic fixtures generated outside the calculation code. Browser acceptance checks the built HTML, six tabs, all eight MARKET reports, region filters, session restoration, mobile layout, English labels and messages, cargo creation, sale validation, rejection of unregistered ports, planner autosave before blur, reload persistence, print action and confirmations. User-entered text is preserved as entered. Validation against a complete real voyage remains outstanding.
