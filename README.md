...
- **PLANNER** — multiple parcels, loading/discharge rotation, preliminary hold allocation and voyage cost allocation. Each parcel carries a shipment passport (SF basis, source, date, BCSN) with its own version history; cargo separation rules and technical review are recorded against their sources, and a plan version can be frozen and read back unchanged. A hold on the stowage diagram is a picture, not a control.
...
- **State drafts** — every call shows an arrival and a departure state, and the platform calculates the draft of each from that state's own deadweight: cargo from the stowage plan, bunkers chained from the intake figure through the voyage consumption, mean draft from the load line with the berth density correction. The table is folded away by default into one block whose line carries the deepest state and the tightest margin, or leads with any state over its berth limit; a state with no nominated berth counts as unchecked rather than clear. The berth limit is checked against the deepest of the three drafts, and a claim raised against a calculated figure says so. Fresh water, ballast and constant are held at the entered figure; they are not modelled along the voyage.
- **SALE / Sales in voyage** — create and edit sales, then add several to PLANNER together using search, loading/discharge filters and shipment-window overlap. Selections survive filtering; already-added sales cannot be added again. A batch with an invalid sale makes no partial addition. PLANNER shows the shipment window and single-line unit headings; clicking the sale number opens and highlights its record in SALE, while the cargo name opens its shipment passport. Existing inclusion-checkbox behavior is unchanged.
- **Voyage map** — folded by default like Intake Calculator, with mouse and keyboard controls. Opening the map or resizing the window refits its frame; Zoom and Fit remain available. Distance tables and leg inputs stay outside the fold. Routine incomplete-input banners in section 4 are removed, but required-input validation remains: no budget is calculated without a hire rate.
- **MARKET** — 30 archived reports, including 22 added IFCHOR GALBRAITHS reports; the latest is dated 2 September 2026. The eight earlier summaries are preserved. Four source reports have conflicting 2025/2026 dates and are explicitly marked "date unconfirmed"; their printed dates are retained pending confirmation. Report dates, regional filtering, broker rate guidance and publication-time commentary work offline. Read-only, not a live feed, with no automatic updates or independent verification. Browsing does not change voyage inputs. Report and region selection persist for the browser session.
...
node platform/sales-browser-check.cjs
```

Rendering is measured, not guessed: building the stowage section of an eighteen-state voyage takes about 0.8 ms, and a re-render in the browser is dominated by replacing the DOM rather than by the calculation. Number formatting reuses one `Intl.NumberFormat` per digit count; the planner check runs once per render and is shared by the section and the print evidence.
...
The current release passes 201 automated tests. A targeted sales browser check covers batch selection, search and port/date filters, invalid/unknown dates, atomic addition, duplicate prevention, save/reload, direct SALE navigation and editing, shipment passports, keyboard focus, desktop/mobile layout and print visibility. A targeted map browser check covers closed defaults, mouse/keyboard toggles, Zoom/Fit, resizing, preserved fold state during recalculation, reload, mobile layout and missing-hire validation. Run `node platform/map-browser-check.cjs index.html` with Playwright available. The MARKET import also passed a targeted browser check of all 30 reports and all regional filters, date warnings, image-transcribed rates, session restoration, mobile layout and unchanged voyage data. The broader browser script passed MARKET but stopped at an outdated VESSEL heading expectation; its remaining steps are not claimed as verified for this release. The financial engine was previously checked against 240 independent rational-arithmetic fixtures generated outside the calculation code. Validation against a complete real voyage remains outstanding.
